"""One-time helper: extract unit costs from Tyler's bid spreadsheet.
...
Tyler's UNIT LABOR dollars by dividing by the bricklayer wage total
($108.02). This makes `hours_per_unit * wage.total` reproduce Tyler's exact
...
stdout, so you can do:
    python scripts/extract_unit_costs_from_tyler.py > /tmp/entries.py
"""
...
def main() -> int:
    ...
    print("# === Generated by scripts/extract_unit_costs_from_tyler.py ===")
    print('# trade="bricklayer" + hours_per_unit = tyler_unit_labor_$ / 108.02')
    print("# reproduces Tyler's unit-labor dollars exactly. Material is taken")
